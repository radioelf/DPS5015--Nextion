...
class Instrument():
    ...
    def write_registers(self, registeraddress, values):
        ...
        if not isinstance(values, list):
            raise TypeError('El "parámetro de valores" debe ser una lista. Dado: {0!r}'.format(values))
    ...
    def _performCommand(self, functioncode, payloadToSlave):
        ...
        if self.precalculate_read_size:
            try:
                ...
                if self.debug:
                    template = 'MinimalModbus modo debug. No se puede precalcular la longitud de la respuesta para Modbus {} modo. ' + \
                        'leeidos {} bytes. pedidos: {!r}'
    ...
    def _communicate(self, request, number_of_bytes_to_read):
        ...
        if self.debug:
            _print_out('\nMinimalModbus modo debug. Escribir en el instrumento (esperando {} bytes back): {!r} ({})'. \
        ...
        if time_since_read < minimum_silent_period:
            ...
            if self.debug:
                template = 'MinimalModbus modo debug. En reposor {:.1f} ms. ' + \
                        'Minimo periodo de silencia: {:.1f} ms, tiempo desde de leer: {:.1f} ms.'
            ...
            template = 'MinimalModbus modo debug. Sin sleep antes de escribir. ' + \
                'Tiempo transcurrido desde la lectura anterior: {:.1f} ms, periodo minimo de silencio: {:.2f} ms.'
            text = template.format(
                time_since_read * _SECONDS_TO_MILLISECONDS,
                minimum_silent_period * _SECONDS_TO_MILLISECONDS)
            _print_out(text)
        ...
        if self.handle_local_echo:
            ...
            if self.debug:
                template = 'MinimalModbus modo debug. Discarding this local echo: {!r} ({} bytes).' 
        ...
        if self.debug:
            template = 'MinimalModbus modo debug. Respuesta del instrumento: {!r} ({}) ({} bytes), ' + \
                'tiempo de ida y vuelta: {:.1f} ms. Ajuste del tiempo de espera: {:.1f} ms.\n'
            text = template.format(
                answer,
                _hexlify(answer),
                len(answer),
        ...
        if len(answer) == 0:
            raise IOError('Sin comunicación con el instrumento (sin respuesta)')
...
def _extractPayload(response, slaveaddress, mode, functioncode):
    ...
    * ASCII Mode: header (:) + slaveaddress byte + functioncode byte + payloaddata + LRC (which is two characters) + footer (CRLF)
    ...
    if receivedChecksum != calculatedChecksum:
        template = 'Checksum error en modo {} : {!r} en lugar de {!r} . la respuesta es: {!r} (respuesta simplificada: {!r})'
        text = template.format(
                mode,
                receivedChecksum,
                calculatedChecksum,
...
def _predictResponseSize(mode, functioncode, payloadToSlave):
    ...
    if functioncode in [5, 6, 15, 16]:
        ...
        raise ValueError('Código de función incorrecto: {}. El payload es: {!r}'.format( \
...
def _calculateCrcString(inputstring):
    ...
    #print ("Valores para el calculo del CRC %s " % _hexlify(inputstring))
    #print ("El CRC es: %s " % _hexlify(_numToTwoByteString(register, LsbFirst=True)))
...
def _print_out(inputstring):
    ...
    """
    _checkString(inputstring, description='Imprimir el string')
